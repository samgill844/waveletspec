...
for code in ['spectrum']:
	Trange = np.arange(4000,8050,250)
	Vrange = [None]
	...
	ref = np.linspace(350.0,800.,2**18)
...
#		continue
	    ...
	    np.save(str(teff)+'_'+str(0)+'_'+str(MH)+'_'+str(logg),tmp)
	...
	waveobs = np.linspace(350.0,800.0,131072) # wavelength scale for spectra - leave
	# table params
	teff=[]
	mh=[]
	logg=[]
	...
	for i in range(0,len(files)): # populate spectra and table parameters
		...
		Vsin.append(0) # added vsini for quick grid
